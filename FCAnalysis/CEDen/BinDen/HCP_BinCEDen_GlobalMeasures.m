...
VarList={'wCEDen','GE'}; %,'C'};
VarList2Show={'\textbf{wCE Density}','\textbf{GE}'};
    ...
    for Atlas = AtlasList
            ...
            for s_cnt = 1:numel(SubList)
                ...
                if strcmp(TarVar{1},'wCEDen')
                    ...
                    ndf(s_cnt,1)   = eval(['Naive.b.' TarVar{1}]);
                    hmedf(s_cnt,1) = eval(['ME.b.'   TarVar{1}]);
                    hcrdf(s_cnt,1) = eval(['CR.b.'   TarVar{1}]);
                end
...
export_fig(fh,'Figs/Global_wCEDen.pdf');