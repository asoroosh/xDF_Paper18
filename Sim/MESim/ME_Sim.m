...
tsDir = ['/storage/essicd/data/HCP/Soroosh/ACAnal/MESim/R_tsSim_t' num2str(t_cnt) '_r' num2str(rho_cnt) '/'];
if exist(tsDir,'dir')~=7; mkdir(tsDir); end
...
for ac_cnt=1:ncAC
	...
	dlmwrite([tsDir 'ts_AC' num2str(ac_cnt) '_' num2str(T_list(t_cnt)) '_r' num2str(rho_list(rho_cnt).*100) '_' num2str(i_cnt) '.txt'],ts)
...
%        z_r_ts1(ac0,ac1)  = Stat_tmp.z.rz(1,2);
	...
	[v_ch_tmp,z_ch_tmp,p_ch_tmp] = CheltonBCF(ts,T,4);
        v_ch4(ac0,ac1) = v_ch_tmp(1,2);
        z_ch4(ac0,ac1) = z_ch_tmp(1,2);
        p_ch4(ac0,ac1) = p_ch_tmp(1,2);
	...
	%---------------------------
        ...
        %-- CR with curbing factor T/5; exactly what Pyper suggests
        [v_cr_tmp,z_cr_tmp,p_cr_tmp] = CRBCF(ts,T,4);
        v_cr4(ac0,ac1) = v_cr_tmp(1,2);
        z_cr4(ac0,ac1) = z_cr_tmp(1,2);
        p_cr4(ac0,ac1) = p_cr_tmp(1,2);
