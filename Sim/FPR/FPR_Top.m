...
p_MEts1 = Stat.p.f_Pval;
p_MEts1(1:Np+1:end) = 1;
%----ME
[V,Stat]    = PearCorrVarEst(simts,Tp,'taper','curb',T/4);
p_MEtc4 = Stat.p.f_Pval;
p_MEtc4(1:Np+1:end) = 1;
%----ME
[V,Stat]    = PearCorrVarEst(simts,Tp);
p_ME = Stat.p.f_Pval;
p_ME(1:Np+1:end) = 1;
%----CR
[~,~,p_CR]  = CRBCF(simts,Tp);
...
p_AR1MC(1:Np+1:end) = 0;
%----Fox
[~,~,p_Fox]   = FoxBCF(simts,Tp);
p_Fox(1:Np+1:end) = 0;
%----Naive
[~,p_Naive]   = corr(simts');
p_Naive(1:Np+1:end) = 0;

EstsLables = {'ME','MEts1','MEtc4','CR','CH','AR1','AR1MC','Fox','Naive'};
...
%----SAVE
save(['R/Sen_Spec_' num2str(i_cnt) '.mat'],'Sen','Spc','Acc','H','TP','FP','FN','TN');
